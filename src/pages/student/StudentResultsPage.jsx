...
export default function StudentResultsPage() {
  ...
  useEffect(() => {
    ...
    setExamResults(myExams)
    setProjectResults(myProjects)
  }, [])
  ...
  const allScores = useMemo(() => {
    ...
    const projectScores = projectResults.map(p => p.score)
    return [...examScores, ...projectScores]
  ...
  const bestScore =
    totalTasks > 0 ? Math.max(...allScores) : 0

  const worstScore =
    totalTasks > 0 ? Math.min(...allScores) : 0
  ...
  const generateFeedback = (score, total) => {
    ...
    if (percent >= 90)
      return "Zo‘r natija! Siz mavzuni mukammal o‘zlashtirgansiz."
    if (percent >= 75)
      return "Yaxshi! Biroz mashq qilsangiz yanada yuqori natija."
    if (percent >= 60)
      return "O‘rtacha natija. Ba’zi mavzularni qayta ko‘rib chiqing."
